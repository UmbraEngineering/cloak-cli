...
if (process.argv[2] === 'completion') {
	...
	return tabtab.complete('cloak', function(err, data) {
		if (err || ! data) {
			return;
		}
		...
		if (data.words === 2) {
			switch (data.prev) {
				...
				case 'create':
				case 'destroy':
					return tabtab.log([ 'model', 'router', 'view' ], data);
				break;
		...
		if (data.words === 3) {
			switch (data.prev) {
				...
				case 'router':
					return tabtab.log(jsFiles('routers'), data);
				break;

				case 'view':
					return tabtab.log(jsFiles('views'), data);
				break;
			}
		}
	});
